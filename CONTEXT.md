...
Last updated: June 24, 2026 (Console redesign in progress on branch
design/liquid-glass, commit aa07ed4. Primary green locked: #3d7a42 console
accent, #74cc7c mint/dark sibling. Token layer shipped. All pages except
/apps/[id] and /fleet/devices/[id] converted to CSS variable tokens + inline
styles. Tailwind v4 purging root cause diagnosed and fixed.)
...
4. Make the primary-green call (lime #7dd94a vs hunter ~#355E3B + core
   neutrals) somewhere in this window. Bounded, NOT the full brand session.
   NEXT PRIORITY.
...
### In progress (June 24, 2026 -- console redesign)
Branch: design/liquid-glass. Final commit this session: aa07ed4.
Chip driving Claude Code. Surface-by-surface on feature branch, Vercel
preview per surface before merge to main.

**Root cause diagnosed (Tailwind v4 purging):** Utility classes that appear
only in newly-added/modified files and are not referenced elsewhere in the
bundle are purged from the production CSS. Layout classes (px-6, gap-4,
rounded-2xl, etc.) disappeared in the Vercel build. Fix: all structural
layout, spacing, and color styles moved to inline style props. Inline styles
cannot be purged. This is now a standing rule for all new work.
...
- layout.tsx: inline theme script (no-flash OS-follow), data-theme="light"
  SSR default, ThemeToggle in sidebar footer.

**Status color semantics applied across all converted pages:**
- Outdated/patchable: amber (var(--st-outdated)). Never red.
- Lagging (resolver state, vendor ahead): red (var(--st-lagging)).
  Reserved for this state only. NOT yet rendered (app detail not converted).
- Unknown: gray (var(--st-unknown)).
- Current: green (var(--st-current)).
- Emoji pills removed. Dot/badge pills used throughout.

**Pages converted (color tokens + inline styles):**
- src/app/patches/page.tsx (Patch History)
- src/app/catalog/page.tsx
- src/app/devices/page.tsx
- src/app/fleet/page.tsx
- src/app/reports/page.tsx
- src/app/settings/page.tsx and sub-pages (organization, enrollment,
  labels, jamf)
- src/app/alerts/page.tsx
- src/app/orchard/page.tsx (Cultivation)
- src/app/login/page.tsx
- src/components/HomePageInner.tsx (App Inventory)
- src/components/AppCard.tsx
- src/components/PatchStatusBadge.tsx
- src/components/SearchBar.tsx
- src/components/Sidebar.tsx (ThemeToggle added to footer)

**Not yet converted:**
- src/app/apps/[id]/page.tsx (App detail) -- build against
  orchardpatch-app-detail.html reference. Includes resolver version module.
- src/app/fleet/devices/[id]/page.tsx (Device detail)
- Dashboard (/dashboard) -- full design pass per orchardpatch-console-master.html
...
- Console UI redesign: IN PROGRESS on branch design/liquid-glass (commit
  aa07ed4). Token layer shipped. Most pages converted to tokens + inline
  styles. App detail, device detail, and Dashboard full design pass remain.
  See "In progress" section above for full state.

### Not yet built (priority order)
1. Console redesign: complete remaining surfaces (Dashboard full pass, App
   detail against orchardpatch-app-detail.html + resolver version module,
   Device detail). Review each on Vercel preview before merge to main.
2. Demo video + polished repo.
3. Outreach (MacAdmins Slack + contribution-first Installomator maintainer
   contact).
...
- Console UI redesign: IN PROGRESS (branch design/liquid-glass, commit
  aa07ed4). Hard constraint still holds: zero hardcoded hex in components,
  all layout/spacing via inline styles (Tailwind v4 purging). Remaining
  surfaces: Dashboard full pass, App detail (resolver version module),
  Device detail.
...
1. Console redesign: remaining surfaces on branch design/liquid-glass.
   a. Dashboard full design pass (against orchardpatch-console-master.html).
   b. App detail (against orchardpatch-app-detail.html + version-module-states.html).
      This is where the resolver version module (current/patchable/lagging/unknown)
      gets built. The lagging state is the demo differentiator.
   c. Device detail.
   d. Review each surface on Vercel preview before merging to main.
2. Demo video + polished repo.
3. Outreach.
NOTE: All new component work uses inline style props for layout/spacing/color.
No Tailwind utility classes for these. See standing rule in AI workflow section.
...
## Lessons learned (June 24 -- console redesign build)
- Tailwind v4 purges utility classes that appear only in newly-added or
  heavily-modified files and are not referenced elsewhere in the bundle.
  Layout classes (px-6, gap-4, rounded-2xl, grid-cols-2) disappeared in
  the Vercel build while looking fine locally. The fix is structural: move
  all layout, spacing, and color to inline style props. Inline styles are
  immune to purging by definition.
- Dark mode renders correctly before light mode when the glass effect relies
  on the backdrop-filter blur. Dark background provides contrast; light-on-
  light does not. If dark looks right and light collapses, check the ambient
  background layer and card surface stacking first.
- DevTools Sources panel shows that code is present. DevTools Computed panel
  shows what the browser actually resolved. These are different questions.
  Diagnosing CSS variable resolution requires Computed, not Sources.
- When manual browser inspection creates a bottleneck, the right move is a
  self-contained diagnostic page (public/design-debug.html) that renders
  every token as a swatch with its resolved value. Build it once; use it
  forever on this codebase.
